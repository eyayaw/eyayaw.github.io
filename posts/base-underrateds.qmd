...
Of course, `{here}` is more powerful (has sophisticated project root resolving mechanisms, robust to in-session working dir changes (with setwd()), and [more](https://here.r-lib.org/articles/here.html)). But if you are using projects and work with (project) relative paths, `file.path` may do the job just fine. The project-relative path construction feature of `{here}` (i.e., with `here::here`) is the comparable part to `file.path`.
